...
Our hypothesis is that in general people value more frequently the movies they like. In addition, poorly rated films are generally less viewed (people are carried away by the opinion of the majority). If we look again at the distribution chart of average ratings per film, we see a clear bias to the right. There are few films with average ratings below 2.5. Our final model therefore had few examples to learn from and, as we will see later, it tends to overvalue movies with low averages. 
...
For example, the following table shows the 10 films released in 1940 with the most ratings:
...
As might be expected, there is great variability in the number of ratings per *movie_year*: from 32 for films released in 1917 (2), to around 800,000 for films released in 1995 (362). Regularization here is also necessary.
...
1. The aforementioned durability effect.
2. The blockbuster effect.

Both are included in the final model in combination, through the movie year effect ($b_{my}$),
...
It is also observed that the Film-Noir, Documentary, IMAX and War genres have a higher average rating. It is feasible to say that these are niche genres, which are probably valued by users who have a positive tendency towards them.

Horror movies, meanwhile, have low average ratings. In the middle we find genres that can be classified as niche (Western, Musical, Mystery), and popular such as Romance, Adventure, Thriller, Action, Comedy and Drama.
...
Another way to see this variability is with a box plot. Here we have filtered the data of user 559269, who has rated more than 6,600 films (probably, it is the user assigned to those responsible for the movielens site):
