...
During model development, we experimented with the genre effect in two different ways. The first approach was to separate the movie genres by row, and add them to the model in the same way that the movie, user, release year, and rating year effects were added: $Y_{u,i} = \frac{1}{n}\sum\left(mu + b_i + b_u + b_{my} + b_{ry} + b_g \right) + \epsilon_{u,i}$
...
3. real correlations between movie genres,
2. weak correlations, which are due to the fact that there are few examples in the data.  
...
```{r creating_rating_matrix, echo=TRUE}
...
Below we show the code as an example (commented, to avoid its execution):

```{r recomenderlab_exmple_commented, echo=TRUE, message=FALSE, warning=FALSE}
...
$$Y_{u,i} = \frac{1}{n}\sum\left(mu + b_i + b_u + b_{my} + b_{ry} + b_g \right) + \epsilon_{u,i}$$
The first attempt to incorporate genders into the model was based on the separation into rows of the multi-categorical variable called *genres* (see the beginning of the section 2.2.4.1 to review the details of the code). As a reminder, we include here the example tables that we have already seen in the introduction:
