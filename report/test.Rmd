...
```{r separate_genders_by_row, echo=FALSE, message=FALSE, warning=FALSE}
...
Our model does not differ much from the one developed in the course documentation, although it includes the time trends represented by the effects of the year the movies were released, and the year of rating. On the other hand, if it does not include the effect of the genre of the films, it is because its inclusion did not improve the results.

It is a linear model capable of working with the complete edx dataset. The advantage of this approach is that despite being a very large dataset, it keeps the calculation times within reasonable ranges. The downside is that both user and time effects are approximate, and the model does not capture the non-linear nature of latent factors. Despite these disadvantages, the model yields an RMSE in line with the primary objective: to be below 0.86490.
...
To summarize, below we show the code by which the model that incorporates the average rating and the movie and user effects is built:

$$Y_{u,i} = mu + b_i + b_u + \epsilon_{u,i}$$

```{r mu_bi_bu_model, echo=TRUE}
...
```{r lamda_best_tune_1, echo=FALSE}
